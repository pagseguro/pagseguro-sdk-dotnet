...
namespace Uol.PagSeguro.Service
{
    ...
    public static class CancelService
    {
        ...
        public static CancelRequestResponse RequestCancel(Credentials credentials, string transactionCode)
        {
            ...
            try {
                    ...
                    PagSeguroConfiguration.CancelUri.AbsoluteUri, BuildCancelURL(credentials, transactionCode)))
                {
                    ...
                    using (XmlReader reader = XmlReader.Create(response.GetResponseStream()))
                    {
                        ...
                        CancelRequestResponse cancel = new CancelRequestResponse();
                        CancelSerializer.Read(reader, cancel);
                        PagSeguroTrace.Info(String.Format(CultureInfo.InvariantCulture, "PagSeguroCancelService.createRequest({0}) - end", cancel.ToString()));
                        return cancel;
                    }
                }
                ...
                PagSeguroTrace.Error(String.Format(CultureInfo.InvariantCulture, "PagSeguroCancelService.createRequest() - error {0}", pse));
                throw pse;
            }
        }
